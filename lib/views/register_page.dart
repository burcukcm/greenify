...
class _RegisterPageState extends State<RegisterPage> {
  ...
  Future<void> _registerUser() async {
    ...
    if (email.isEmpty || password.isEmpty || passwordAgain.isEmpty) {
      setState(() {
        errorMessage = AppStrings.fillAllFields;
      });
      return;
    }
    ...
    if (password != passwordAgain) {
      setState(() {
        errorMessage = AppStrings.passwordsNotMatch;
      });
      return;
    }
    ...
    final result = await Auth().createUser(email: email, password: password);

    if (result == null) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text(AppStrings.regisSuccessful)),
      );
